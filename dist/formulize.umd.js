    ...
    ***************************************************************************** */
    ...
    var UIElementHelper = /** @class */ (function () {
        ...
        UIElementHelper.getDragElement = function (id) {
            return jquery("<div class=\"" + id + "-drag\"></div>")[0];
        };
        UIElementHelper.getCursorElement = function (id) {
            return jquery("<div class=\"" + id + "-cursor\"></div>")[0];
        };
        UIElementHelper.getUnitElement = function (id, value) {
            var unitElem = jquery("<div class=\"" + id + "-item " + id + "-unit\"></div>");
            UIElementHelper.setUnitValue(id, unitElem[0], value);
            return unitElem[0];
        };
        UIElementHelper.getUnitDecimalElement = function (id, side, value) {
            return jquery("<span class=\"" + id + "-" + side + " " + id + "-decimal-highlight\">" + (value || '') + "</span>")[0];
        };
        UIElementHelper.getOperatorElement = function (id, value) {
            return jquery("<div class=\"" + id + "-item " + id + "-operator\">" + (value || '').toLowerCase() + "</div>")[0];
        };
        UIElementHelper.getTextBoxElement = function (id) {
            return jquery("<textarea id=\"" + id + "-text\" name=\"" + id + "-text\" class=\"" + id + "-text\"></textarea>")[0];
        };
        UIElementHelper.setUnitValue = function (id, elem, value) {
            ...
            var prefix = jquery(UIElementHelper.getUnitDecimalElement(id, 'prefix', split[0]));
            prefix.appendTo(jquery(elem));
            if (split[1] === undefined)
                return;
            var suffix = jquery(UIElementHelper.getUnitDecimalElement(id, 'suffix', "." + split[1]));
            suffix.appendTo(jquery(elem));
        };
        UIElementHelper.isElementType = function (id, type, elem) {
            return elem
                ? jquery(elem).hasClass(id + "-" + type)
                : false;
        };
        ...
        UIHelper.getDataValue = function (data) {
            if (!UIHelper.isDOM(data))
                return StringHelper$1.isNumeric(data)
                    ...
                    : data;
            var value = jquery(data).data('value') || jquery(data).text();
    ...
    var UIDom = /** @class */ (function () {
        ...
        UIDom.prototype.initializeDOM = function () {
            ...
            this.wrapper = jquery(this.elem);
            this.wrapper.addClass(this.options.id + "-wrapper");
            this.container = jquery("<div class=\"" + this.options.id + "-container\"></div>");
            this.container.appendTo(this.wrapper);
            this.statusBox = jquery("<div class=\"" + this.options.id + "-alert\">" + this.options.text.formula + "</div>");
            this.statusBox.insertBefore(this.container);
            this.textBox = jquery(UIElementHelper.getTextBoxElement(this.options.id));
        ...
        UIDom.prototype.isAlreadyInitialized = function () {
            return jquery(this.elem).hasClass(this.options.id + "-wrapper");
        };
        UIDom.prototype.attachEvents = function () {
            throw new Error('method not implemented');
        };
        UIDom.prototype.getPrevUnit = function (elem) {
            var prevElement = jquery(elem).prev();
        ...
        UIDom.prototype.mergeUnit = function (baseElem) {
            var _this = this;
            var prevElem = jquery(this.getPrevUnit(baseElem));
            var nextElem = jquery(this.getNextUnit(baseElem));
    ...
    var UIPipe = /** @class */ (function (_super) {
        ...
        UIPipe.prototype.pipeTrigger = function (name, value) {
            jquery(this.elem).triggerHandler(this.options.id + "." + name, value);
    ...
    var UIManager = /** @class */ (function (_super) {
        ...
        UIManager.prototype.pick = function (position) {
            ...
            this.removeCursor();
            this.cursor = jquery(UIElementHelper.getCursorElement(this.options.id));
        ...
        UIManager.prototype.moveDrag = function (position) {
            ...
            positions.sort();
            var dragElem = jquery(UIElementHelper.getDragElement(this.options.id));
        ...
        UIManager.prototype.findClosestUnit = function (position) {
            var unitPositions = this.container
                ...
                elem: elem,
                x: jquery(elem).position().left + jquery(elem).outerWidth(),
                y: jquery(elem).position().top
            }); });
        ...
        UIManager.prototype.moveCursorBefore = function (elem) {
            if (!jquery(elem).length)
                return;
            this.cursor.insertBefore(jquery(elem));
        };
        UIManager.prototype.moveCursorAfter = function (elem) {
            if (!jquery(elem).length)
                return;
            this.cursor.insertAfter(jquery(elem));
        };
        ...
        UIManager.prototype.insert = function (data, position) {
            ...
            if (!UIHelper.isDOM(pipedData))
                return;
            var insertElem = jquery(pipedData);
        ...
        UIManager.prototype.insertValue = function (value) {
            ...
            if (FormulizeTokenHelper.isNumeric(value)) {
                var unitElem = jquery(UIElementHelper.getUnitElement(this.options.id, value));
                if (this.dragElem.length) {
                    this.cursor.insertBefore(this.dragElem);
                    this.dragElem.remove();
                }
            ...
            var operatorElem = jquery(UIElementHelper.getOperatorElement(this.options.id, value));
            if (this.cursor && this.cursor.length)
    ...
    function pluginBinder() {
        ...
        jquery.fn.formulize = Object.assign(function (options) {
            this
                .toArray()
                .forEach(function (elem) {
                jquery(elem).data('$formulize', new UI(elem, options));
            });
            return this;
        ...
        reflectedMethodNames
            ...
            jquery.fn[binder.name] = binder.func;
        });
    }
